...
description: 'KidaPay can send webhook events that notify your application any time order status has been paid.'
icon: 'webhook'
---
...
4. KidaPay sends payment notification while your application returns response 200 (OK) HTTP status code. You should respond in json format with content: 
```
 {
     "status": 200
 }
...
<tbody>
	...
	<tr>
		<td>token</td>
		<td><code>string</code></td>
		<td>Your token provided by CreateOrder to validate Payment Callback.</td>
	</tr>
	<tr>
		<td>meta</td>
...
The KidaPay server attempts to send callbacks multiple times until the send is either successful or the KidaPay server gives up. The KidaPay server attempts retries using an exponential back on the following schedule:
